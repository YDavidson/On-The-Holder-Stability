...
class SortConv(MessagePassing):
    ...
    def reset_parameters(self):
        ...
        self.lin_project.weight = nn.Parameter(self.lin_project.weight / (torch.norm(self.lin_project.weight, dim=1, keepdim=True) * self.out_dim))
        if self.collapse_method == 'vector':
          torch.nn.init.xavier_normal_(self.lin_collapse.weight)
          self.lin_collapse.weight = nn.Parameter(self.lin_collapse.weight / (torch.norm(self.lin_collapse.weight, dim=1, keepdim=True) * self.max_nodes))
        elif self.collapse_method == 'matrix':
          torch.nn.init.normal_(self.lin_collapse)
          self.lin_collapse.weight = nn.Parameter(self.lin_collapse / (torch.norm(self.lin_collapse, dim=1, keepdim=True) * self.max_nodes))
    ...
    def message(self, x_j, edge_attrs=None):
       ...
       return self.lin_project(x_j + edge_attrs)
