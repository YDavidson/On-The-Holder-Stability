...
from pooling import ZINCSortGlobalConv, ZincAdaptiveReLUGlobalConv
...
class GNN(torch.nn.Module):
    ...
    def __init__(self, num_tasks, num_layer=5, in_dim=300, emb_dim=300,
                 ...
                 feature_encoder=lambda x: x, conv_kwargs={}, pool_kwargs={}):
        ...
        self.out_dim = self.emb_dim if self.JK in ['last', 'sum'] else self.emb_dim * self.num_layer + in_dim
        ...
        self.gnn_node = GNN_node(num_layer, in_dim, emb_dim, JK=JK, drop_ratio=drop_ratio, residual=residual,
                                 ...
                                 feature_encoder=feature_encoder, conv_kwargs=conv_kwargs)
        ...
        if self.graph_pooling == "sum":
            ...
            self.pool = ZINCSortGlobalConv(emb_dim, emb_dim, **pool_kwargs)
        elif self.graph_pooling == "adaptive_relu":
            self.pool = ZincAdaptiveReLUGlobalConv(emb_dim, emb_dim, **pool_kwargs)
        else:
...
class GNNComplete(GNN):
    def __init__(self, num_tasks, num_layer=5, in_dim=300, emb_dim=300,
        ...
        super(GNNComplete, self).__init__(num_tasks, num_layer, in_dim, emb_dim, gnn_type, num_random_features,
                                          residual, drop_ratio, JK, graph_pooling, feature_encoder, conv_kwargs, pool_kwargs)
...
class DSSnetwork(torch.nn.Module):
    def __init__(self, num_layers, in_dim, emb_dim, num_tasks, feature_encoder, GNNConv, 
                 conv_kwargs, graph_pooling='mean', pool_kwargs={}):
        ...
        if graph_pooling == 'sort':
            self.pool = ZINCSortGlobalConv(emb_dim, emb_dim, **pool_kwargs)
        elif graph_pooling == 'adaptive_relu':
            self.pool = ZincAdaptiveReLUGlobalConv(emb_dim, emb_dim, **pool_kwargs)
        else:
            self.pool = global_mean_pool
        ...
        for i in range(num_layers):
            gnn_list.append(GNNConv(emb_dim if i != 0 else in_dim, emb_dim, **conv_kwargs))
            bn_list.append(torch.nn.BatchNorm1d(emb_dim))

            gnn_sum_list.append(GNNConv(emb_dim if i != 0 else in_dim, emb_dim, **conv_kwargs))
    ...
    def forward(self, batched_data):
        ...
        h_subgraph = subgraph_pool(x, batched_data, self.pool)
        # aggregate to obtain a representation of the graph given the representations of the subgraphs
        h_graph = torch_scatter.scatter(src=h_subgraph, index=batched_data.subgraph_idx_batch, dim=0, reduce="mean")
