...
class GCNWrapper(nn.Module):
  '''
  GCN with specified number of layers.
  '''
  def __init__(self, num_layers, in_dim, embed_dim, out_dim, activation=nn.ReLU(), dropout=0, out_mlp_layers=1,
    ...
    self.convs = GCN(in_channels=in_dim, hidden_channels=embed_dim, out_channels=embed_dim, num_layers=num_layers, dropout=dropout,
                     norm=norm, jk=jk)
