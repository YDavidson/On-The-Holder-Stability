...
class GATWrapper(nn.Module):
  '''
  GAT with specified number of layers.
  '''
  def __init__(self, num_layers, in_dim, embed_dim, out_dim, activation=nn.ReLU(), dropout=0, out_mlp_layers=1,
               norm=None, jk=None, heads=4):
    super().__init__()
    self.num_layers = num_layers
    self.activation = activation
    self.dropout = nn.Dropout(dropout)
    self.convs = GAT(in_channels=in_dim, hidden_channels=embed_dim, out_channels=embed_dim, num_layers=num_layers, dropout=dropout,
                     norm=norm, jk=jk, heads=heads)
